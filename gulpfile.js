...
const cssobfuscator = require("postcss-obfuscator");
...
gulp.task('js', function () {
    return gulp
        ...
        .pipe(closureCompiler({//https://github.com/google/closure-compiler-npm/blob/master/packages/google-closure-compiler/docs/gulp.md
            compilation_level: 'SIMPLE',
            warning_level: 'VERBOSE',
            language_in: 'ECMASCRIPT6_STRICT',
            language_out: 'ECMASCRIPT5_STRICT',
            output_wrapper: '(function(){\n%output%\n}).call(this)',
            js_output_file: 'javascript.min.js'
        }, {
        ...
        /*.pipe(uglify({
            mangle: {
                properties: {
                    // Nếu bạn muốn mangle tất cả các thuộc tính, bạn có thể đặt `true`
                    builtins: true // Bỏ qua các thuộc tính của đối tượng toàn cầu (ví dụ: window, document)
                    // reserved: ['reservedProperty'] // Bỏ qua các thuộc tính được liệt kê trong mảng
                },
            },
            //true: Loại bỏ các hàm không dùng.
            compress: false,                    
            output: {
                beautify: false,//Đặt `beautify` thành `false` để loại bỏ tất cả các khoảng trắng
                comments: false
            }
        }))
        */
    ...
    return gulp
        ...
        .pipe(postcss([         
            cssobfuscator({
                enable: true,
                length: 3,
                srcPath: paths.dist.base.dir,
                desPath: paths.dist.base.dir,
...
gulp.task('build', gulp.series('clean:dist', 'copy:all', 'copy:libs', 'fileinclude', 'scss', 'icons', 'js', 'html'));